...
DEPOT_ID = 0
SPEED = 30  # km/h
START_TIME = datetime(2025, 1, 1, 8, 0)  # Start at 8:00 AM
DISTANCE_METRIC = 'euclidean'  # 'euclidean' or 'manhattan'

# ============= FIXED COORDINATES =============
fixed_pos = {
    DEPOT_ID: (0, 0),  # Depot at origin
    1: (2, 2),        # Close to depot (trap for Greedy), distance ~2.83 km
    2: (12, 0),       # Far along x-axis, early window
    3: (12, 2),       # Near 2, early window
    4: (10, 10),      # Far, mid window
    5: (8, 12)        # Far in y-direction, mid-late window
}

# Time windows (start, end) as datetime objects
time_windows = {
    1: (datetime(2025, 1, 1, 10, 0), datetime(2025, 1, 1, 10, 30)),  # Late window (trap)
    2: (datetime(2025, 1, 1, 8, 30), datetime(2025, 1, 1, 9, 0)),     # Early window
    3: (datetime(2025, 1, 1, 8, 45), datetime(2025, 1, 1, 9, 15)),    # Early window
    4: (datetime(2025, 1, 1, 9, 30), datetime(2025, 1, 1, 10, 0)),    # Mid window
    5: (datetime(2025, 1, 1, 10, 0), datetime(2025, 1, 1, 10, 30))    # Mid-late window
}
...
def plot_search_tree(tree, algorithm, title, filename):
    ...
    plt.title(f"{algorithm} Search Tree - {title}", fontsize=12)
    plt.axis('off')
    plt.tight_layout()
    plt.savefig(filename, dpi=300, bbox_inches='tight')
    plt.show()
...
if __name__ == "__main__":
    ...
    if astar_path:
        ...
        plot_search_tree(astar_tree, "A*", f"A* Delivery Route ({DISTANCE_METRIC.capitalize()})", 
                        f"astar_{DISTANCE_METRIC}_search_tree.png")
        results['astar'] = {
            'Cost': astar_cost,
    ...
    if greedy_path:
        ...
        plot_search_tree(greedy_tree, "Greedy", f"Greedy Delivery Route ({DISTANCE_METRIC.capitalize()})", 
                        f"greedy_{DISTANCE_METRIC}_search_tree.png")
        results['greedy'] = {
            'Cost': greedy_cost,
    ...
    print(f"{'Metric':<20} | {'A*':<15} | {'Greedy':<15}")
    print("-" * 55)
    for metric in ['Cost (km)', 'Nodes Expanded', 'Max Frontier', 'Time (s)']:
        key = 'Cost' if metric == 'Cost (km)' else metric
        
        a_val = results.get('astar', {}).get(key, 'N/A')
        g_val = results.get('greedy', {}).get(key, 'N/A')
        
        if metric == 'Time (s)':
            a_str = f"{a_val:.4f}" if isinstance(a_val, float) else str(a_val)
            g_str = f"{g_val:.4f}" if isinstance(g_val, float) else str(g_val)
            print(f"{metric:<20} | {a_str:<15} | {g_str:<15}")
        else:
